...
public class FriendsListLoader extends AsyncTaskLoader<List<Friend>> {
    ...
    public List<Friend> loadInBackground() {
        ...
        FriendsContract.FriendsColumns.FRIENDS_BIRTHDAY };
        ...
        if(mCursor != null) {
            if(mCursor.moveToFirst()) {
                do {
                    ...
                    String birthday = mCursor.getString(
                            mCursor.getColumnIndex(FriendsContract.FriendsColumns.FRIENDS_BIRTHDAY));
                    Friend friend = new Friend(_id, name, birthday);
                    entries.add(friend);
